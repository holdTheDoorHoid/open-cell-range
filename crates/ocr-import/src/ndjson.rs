//! The native NDJSON capture format from `DESIGN.md` section 3 — the simulator's
//! own on-disk shape, and one of the two formats [`crate`] imports.
//!
//! This module was factored out of `ocr-cli` so the importer and the CLI read and
//! write the *exact same* NDJSON; `ocr-cli`'s `ndjson` module now re-exports this
//! one, so `ocr record` / `ocr replay` are unchanged and `ocr import` understands
//! the same files.
//!
//! One JSON object per line. `t_us`, `rat`, `dir`, `cell` and `msg` follow the
//! design doc's field shape exactly (`rat` is mandatory and authoritative; `msg`
//! is a human-readable decoded-message label). [`ocr_air::AirEvent`] and its
//! [`ocr_air::Payload`] union deliberately do **not** derive `serde` (the engines
//! stay `no_std`, and a real capture's raw-byte decoding is future importer work),
//! so this module defines a LOCAL mirror of exactly the message shapes this
//! simulator's engines can emit, and converts `AirEvent <-> WireEvent` losslessly
//! via the `payload` field.
//!
//! This is deliberately *not* the DESIGN.md `bytes`/`chan`/`arfcn` shape a real
//! hardware capture carries — there is no raw byte encoding here because nothing
//! in this format ever needs decoding; every message the simulator produces is
//! already a decoded struct. Decoding an EXTERNAL capture (GSMTAP pcap today; SCAT,
//! QCSuper, Rayhunter's own output later) into `AirEvent`s is the *other* half of
//! this crate (see [`crate::pcap`] / [`crate::gsmtap`]).
//!
//! `msg` is written for a human (or a future external reader) but is never read
//! back by [`from_line`] / [`read_events`] — `payload` is authoritative, mirroring
//! the design note that a reader falls back from `msg` to decoding the structured
//! content when it doesn't recognise the label.
...
pub enum NdjsonError {
    ...
    Io(io::Error),
    /// A field that must be exactly `expected` bytes (a fixed-width crypto value
    /// like `rand`/`autn`) was a different length.
    BadLength {
        field: &'static str,
        expected: usize,
        got: usize,
...
/// section 3; `payload` is this crate's addition, carrying everything needed to
/// reconstruct the `AirEvent` exactly (see the module docs).
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
...
/// Read a whole NDJSON stream back into events, in order. Blank lines are skipped
/// so a trailing newline in the file doesn't produce a parse error.
...
#[cfg(test)]
mod tests {
    // Only the format-local tests live here. The scenario/monitor round-trip tests
    // stay in `ocr-cli` (they need `ocr-scenario` / `ocr-detect`, which this crate
    // deliberately does not depend on).
    use super::*;
    use ocr_nr::NrRrcMessage;
